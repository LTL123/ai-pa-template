document.addEventListener('DOMContentLoaded', () => {
    ...
    // 仅支持Grade 1 + Physical Education的标准
    const standardsData = {
        '1': {
            'PE': [
        ...
        '2': {
            'PE': [
                ...
                'Aquatics • Show comfort and safety in shallow water.',
                'Aquatics • Float and glide with or without help.',
                'Aquatics • Kick and move with a flotation device.',
                'Aquatics • Follow water safety rules.',
                'Aquatics • Enter and exit the pool safely.',
        ...
        '3': {
            'PE': [
                ...
                'Aquatics • Demonstrate confidence in deeper water.',
                'Aquatics • Perform front and back glide independently.',
                'Aquatics • Show basic breathing and kicking rhythm.',
                'Aquatics • Identify key safety procedures.',
                'Aquatics • Respect others’ space in the pool.',
    ...
    function updateStandardsOptions() {
        standardsSelect.innerHTML = '<option value="">Please select standartds</option>';
        const grade = gradeSelectStandards && gradeSelectStandards.value;
        const subject = subjectSelectStandards && subjectSelectStandards.value;
    ...
    if (gradeSelectStandards && subjectSelectStandards && standardsSelect) {
...
imageUpload.addEventListener('change', function(event) {
    ...
    if (standardsSelect && learningObjectivesTextarea) {
        standardsSelect.addEventListener('change', function() {
            ...
            if (this.value !== "") { // Check if a standard is actually selected
            ...
            try {
                const response = await fetch('https://api.siliconflow.cn/v1/chat/completions', {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [{
                            role: 'user',
                            content: `please help me to improve the learning outcomes as I can statements：${currentContent}. Only provide the improved learning outcome, do not include any other information.`
                        }],
                        stream: false,
                        max_tokens: 4096,
                        temperature: 0.7,
                ...
                if (response.ok) {
                    learningObjectives.value = data.choices[0].message.content;
                    //清除文字前面的空白段落
                    learningObjectives.value = learningObjectives.value.replace(/^\s*[\r\n]/gm, '');
                } else {
...
if (aiEditButton2) {
    aiEditButton2.addEventListener('click', async () => {
        ...
        if (!currentContent) {
            alert('please enter the Essential Questions first');
            return;
        }
        spinner.style.display = 'inline-block';
        ...
        try {
            const response = await fetch('https://api.siliconflow.cn/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        role: 'user',
                        content: `please help me to improve the essential questions for this performance task：${currentContent}. Only provide the improved essential questions, do not include any other information.`
                    }],
                    stream: false,
                    max_tokens: 4096,
                    temperature: 0.7,
            ...
            const data = await response.json();
            if (response.ok) {
                essentialQuestions.value = data.choices[0].message.content;
                essentialQuestions.value = essentialQuestions.value.replace(/^\s*[\r\n]/gm, '');
            } else {
                alert(`API请求失败：${data.error.message}`);
            }
        } catch (error) {
...
if (aiEditButton3) {
    aiEditButton3.addEventListener('click', async () => {
        ...
        if (!currentContent) {
            alert('please enter the backgrounds first');
            return;
        }
        spinner.style.display = 'inline-block';
        ...
        try {
            const response = await fetch('https://api.siliconflow.cn/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        role: 'user',
                        content: `please help me to improve the backgrounds：${currentContent}. Only provide the improved backgrounds, do not include any other information.`
                    }],
                    stream: false,
                    max_tokens: 4096,
                    temperature: 0.7,
            ...
            const data = await response.json();
            if (response.ok) {
                backgrounds.value = data.choices[0].message.content;
                backgrounds.value = backgrounds.value.replace(/^\s*[\r\n]/gm, '');
            } else {
                alert(`API请求失败：${data.error.message}`);
            }
        } catch (error) {
...
if (aiEditButton4) {
    aiEditButton4.addEventListener('click', async () => {
        ...
        if (!currentContent) {
            alert('please enter the PA Description first');
            return;
        }
        spinner.style.display = 'inline-block';
        ...
        try {
            const response = await fetch('https://api.siliconflow.cn/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        role: 'user',
                        content: `please help me to improve the Performance  Description${currentContent}. Only provide the improved PA Description, do not include any other information.`
                    }],
                    stream: false,
                    max_tokens: 4096,
                    temperature: 0.7,
            ...
            const data = await response.json();
            if (response.ok) {
                paDescription.value = data.choices[0].message.content;
                paDescription.value = paDescription.value.replace(/^\s*[\r\n]/gm, '');
            } else {
                alert(`API请求失败：${data.error.message}`);
            }
        } catch (error) {
...
if (aiEditButton5) {
    aiEditButton5.addEventListener('click', async () => {
        ...
        if (!currentContent) {
            alert('please enter the artifacts evidence first');
            return;
        }
        spinner.style.display = 'inline-block';
        ...
        try {
            const response = await fetch('https://api.siliconflow.cn/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        role: 'user',
                        content: `please help me to improve the artifacts, evidence, and final products ${currentContent}. Only provide the improved  artifacts, evidence, and final products, do not include any other information.`
                    }],
                    stream: false,
                    max_tokens: 4096,
                    temperature: 0.7,
            ...
            const data = await response.json();
            if (response.ok) {
                artifactsEvidence.value = data.choices[0].message.content;
                artifactsEvidence.value = artifactsEvidence.value.replace(/^\s*[\r\n]/gm, '');
            } else {
                alert(`API请求失败：${data.error.message}`);
            }
        } catch (error) {
    ...
    console.log('Checking window.jspdf:', window.jspdf);
...
if (downloadPdfButton) {
    ...
    function updateCompetencyIndicators() {
        ...
        if (selectedCompetency && competencyIndicatorsData[selectedCompetency]) {
            ...
            competencyIndicatorsData[selectedCompetency].forEach(indicatorText => {
                ...
                checkbox.id = 'indicator-' + indicatorText.replace(/[^a-zA-Z0-9]/g, "-");   
    ...
    const rubricsData = {
        ...
        "4.2: Maintains focus and balance in the face of stress or uncertainty.": {
            ...
            "Grade 9-12": {
                ...
                "Proficient": "Adapts effective coping strategies (e.g., mindfulness, problem-solving) to maintain focus and emotional balance in stressful or uncertain situations.",
    ...
    if (downloadPdfButton) {
        downloadPdfButton.addEventListener('click', () => {
            ...
            //插入图片
            const base64Image = document.getElementById('base64Output').textContent;
            if (base64Image) {
                const imgData = 'data:image/png;base64,' + base64Image;
                doc.addImage(imgData, 'PNG', 10, yPosition, 100, 100);
                yPosition += 110;
            }
            // 如果没有图片，直接继续添加其他内容
            ...
            try {
                ...
                alert('there is a problem generating the PDF');
            }
        });
    } else {
